...
class RentalCategories extends StatefulWidget {
  const RentalCategories({Key? key}) : super(key: key);

  @override
  State<RentalCategories> createState() => _RentalCategoriesState();
}

class _RentalCategoriesState extends State<RentalCategories> {
  List<Category> categories = [];
  List<Category> filteredCategories = [];
  final TextEditingController _searchController = TextEditingController();
  final FocusNode _searchFocusNode = FocusNode();
  ...
  Future<void> fetchCategories() async {
    try {
      final snapshot =
          await FirebaseFirestore.instance.collection('rentalImages').get();
      setState(() {
        categories =
            snapshot.docs.map((doc) => Category.fromFirestore(doc)).toList();
